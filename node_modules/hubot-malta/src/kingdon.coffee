...
module.exports = (robot) ->
  ...
  robot.respond /find me a team/i, (msg) ->
  ...
  robot.hear ///(#{robot.name}.?\s+)?(.*)///, (res) ->
    ...
    saveIdea = (mat) ->
      ...
      newState = "askAdvPreference"
      robot.brain.set "state:#{username}", newState
      flipFlop username, parentState, parentState, askAdvPreference, match, robot.brain
    return null if flipFlop username, userstate, parentState, saveIdea, match, robot.brain

    parentState = "askAdvPreference"
    askAdvPreference = (mat) ->
      newState = "saveAdvPreference"
      robot.brain.set "state:#{username}", newState
      flipFlop username, newState, parentState, askAdvPreference, match, robot.brain
    return null if flipFlop username, userstate, parentState, askAdvPreference, match, robot.brain
    ...
    saveAdvPreference = (mat) ->
      if mat.match /yes/
        ...
        flipFlop username, newState, newState, saveAdvPreference, match, robot.brain
      else if mat.match /no/
        ...
        res.send "I didn't get that... yes or no?"
        flipFlop username, parentState, parentState, saveAdvPreference, match, robot.brain
