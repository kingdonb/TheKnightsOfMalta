...
module.exports = (robot) ->
  ...
  robot.hear ///(#{robot.name}.?\s+)?(.*)///, (res) ->
    ...
    saveIdea = (mat) ->
      robot.brain.set "ideas:#{username}", mat
      robot.brain.set "state:#{username}", "askAdvPreference"
      res.send "Thanks."
    flipFlop username, userstate, parentState, saveIdea, match, robot.brain

    parentState = "askAdvPreference"
    askAdvPreference = (mat) ->
      res.send "@#{username}, do you want your idea to be listed?"
      newState = "saveAdvPreference"
      robot.brain.set "state:#{username}", ""
      flipFlop username, parentState, newState, askAdvPreference, match, robot.brain
