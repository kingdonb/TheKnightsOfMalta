...
module.exports = (robot) ->
  ...
  robot.hear ///(#{robot.name}.?\s+)?(.*)///, (res) ->
    ...
    if (userstate == parentState)
      userstate2 = robot.brain.get "state:#{username}:#{parentState}"
      if (userstate2 == null)
        robot.brain.set "state:#{username}:#{parentState}", true
      else
        robot.brain.set "state:#{username}:#{parentState}", null
