...
module.exports = (robot) ->
  ...
  robot.respond /(.*)/, (res) ->
    ...
    userstate2 = robot.brain.get "state:#{username}:has an idea"
    if (userstate2 == "on") and (userstate == "has an idea")
      robot.brain.set "state:#{username}:has an idea", true
    else
      robot.brain.set "state:#{username}:has an idea", "off"
      robot.brain.set "ideas:#{username}", res.match[1]
