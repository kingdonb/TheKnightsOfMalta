  ...
  robot.hear ///(#{robot.name}.?\s+)?(.*)///, (res) ->
    ...
    askForIdea = (mat) ->
      if mat.match /yes/
        ...
        robot.brain.set "state:#{username}", "what is idea"
        flipFlop username, userstate, parentState, askForIdea, match, robot.brain
    ...
    saveLanguage = (mat) ->
      ...
      robot.brain.set "state:#{username}", "ask if idea"
      flipFlop username, userstate, parentState, askForIdea, match, robot.brain
