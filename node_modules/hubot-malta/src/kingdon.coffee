...
flipFlop = (user, sta, par, job, mat) ->
  if (sta == par)
    userstate2 = robot.brain.get "state:#{user}:#{par}"
    if (userstate2 == null)
      robot.brain.set "state:#{user}:#{par}", true
    else
      robot.brain.set "state:#{user}:#{par}", null
      job mat
...
module.exports = (robot) ->
  ...
  robot.hear ///(#{robot.name}.?\s+)?(.*)///, (res) ->
    ...
    doTheJob = (mat) ->
      ...
      res.send "Got it."
    flipFlop username, userstate, parentState, doTheJob, match
