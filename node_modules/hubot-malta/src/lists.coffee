...
#
  ...
  robot.respond /list( project)? ideas/i, (msg) ->
    ...
    unless ideas.length == 0
      ...
      t = "No ideas so far!\n"
      t = t + "Try 'tebot list me'"
      msg.send
      return false
    if c > 0
      msg.send "Say '#{robot.name} idea #N' if you want to join a team, or say '#{robot.name} I have an idea' to list your own idea"
      msg.send t
    else
      msg.send "No ideas in my brain.  Try '#{robot.name} find me a team' or '#{robot.name} i have an idea'"
